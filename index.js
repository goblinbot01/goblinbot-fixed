...
// Check does CONFIG, USERDATA and SERVERDATA files exist. (Firstly we will require FS). Define first the error.
...
ENOENT.prototype = new Error();
ENOENT.prototype.name = "ENOENT";
const fs = require("fs");
if (!fs.existsSync("./config.json")) {
  console.log("\x1b[31m%s\x1b[0m", "GoblinBot ERR!: ENOENT: config.json file might not be found. The bot can not continue. (No such file or directory)")
  throw new ENOENT("No such file or directory (config.json). More in console log.")
}
if (!fs.existsSync("./userData/userData.json")) {
  console.log("\x1b[31m%s\x1b[0m", "GoblinBot ERR!: ENOENT: userData/userData.json file might not be found. The bot can not continue. (No such file or directory)")
  throw new ENOENT("No such file or directory (userData/userData.json). More in console log.")
}
if (!fs.existsSync("./userData/serverData.json")) {
  console.log("\x1b[31m%s\x1b[0m", "GoblinBot ERR!: ENOENT: userData/serverData.json file might not be found. The bot can not continue. (No such file or directory)")
  throw new ENOENT('No such file or directory (userData/serverData.json). More in console log.');
}
// After we authenticated file accessiblity, we may proceed.
...
var welcomeembed = embedCreate(3447003,"**Thundertides Games on Modd.io** is the official server for Thundertides Games, based on creation base on Modd.io. \n \n If you have any ideas, feel free to publish them! \n \n Rules: \n 1. Keep everything SFW. \n 2. Please do not advertise other Discord servers. \n 3. Do not spam your content. \n 4. Please use English. \n 5. Reporting bugs dedicating to one of Thundertides Games must be: \n   A. Detailed; \n   B. Have good context; \n   C. How to reproduce it. \n \n *Any staff of that server is free to kick or ban at any time, without warning the user*.", "Welcome to the official server for Thundertides Games on Modd.io!")
...
client.on("message", function(message) {
  ...
  switch (args[0]) { // Command loop
    ...
    case "slots":
      ...
      break;
    case "temp":
      message.channel.send("The current loop value is " + loop);
      break;
    case "meme":
